@echo off
rem Launch the native messaging host. %CLZ_RUNTIME% wins, then bun, then node.
...
if defined CLZ_RUNTIME if exist "%CLZ_RUNTIME%" (
    "%CLZ_RUNTIME%" "%CLZ_ROOT%\lib\host\index.js" %*
    exit /b
)
where /q bun.exe && (
    bun "%CLZ_ROOT%\lib\host\index.js" %*
    exit /b
)
node "%CLZ_ROOT%\lib\host\index.js" %*
exit /b
